...
class DBHelper {
  static Future<sql.Database> database() async {
    ...
    return sql.openDatabase(path.join(dbPath, 'searches.db'),
      ...
      await db.execute(
          'CREATE TABLE user_searches(searchText TEXT PRIMARY KEY, id TEXT, name TEXT, text TEXT, hasTwoSides BIT)');
      await db.execute(
          'CREATE TABLE search_images(searchText TEXT PRIMARY KEY, id TEXT, frontImage TEXT, backImage TEXT)');
      await db.execute(
          'CREATE TABLE search_prices(searchText TEXT PRIMARY KEY, id TEXT, tcg TEXT, tcgFoil TEXT, cdm TEXT, cdmFoil TEXT)');
      // await db.execute(
